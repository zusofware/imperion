...
void ImportExportDatabaseDialog::messageReceived(Message *msg)
{
    if(msg->isTypeCommand(MSG_TYPE::DATABASE, MSG_COMMAND::EXPORT)) {
        if(msg->isSuccess()) {
            if(!mIsGDrive) {
                auto filename = QFileDialog::getSaveFileName(this, tr("Save exported database"), QDir::homePath(), "*.iprn");
                if(!filename.isEmpty()) {
                    if(!filename.endsWith(".iprn")) filename += ".iprn";
                    QFile file(filename);
...
{
    const QString &filename = QFileDialog::getOpenFileName(this, tr("Import database"), QDir::homePath(), "*.iprn");
    if(!filename.isEmpty()) {
        GuiUtil::enableWidget(false, QList<QWidget*>{ui->pushExportToFile, ui->pushExportToGDrive, ui->pushImportFile, ui->pushImportGDrive});
