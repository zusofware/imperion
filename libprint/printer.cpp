...
void Printer::print(const QString &printName, const QString &data, int type, uint16_t vendorId, uint16_t produckId)
{
    ...
    wcscpy_s(docName, 100, L"Imperion Document");
...
#ifndef NO_PRINTER_SPOOL
        int jobId = 0;
        jobId = cupsCreateJob(CUPS_HTTP_DEFAULT, printName.toStdString().c_str(), "Imperion print", 0, NULL);
        if(jobId > 0) {
            cupsStartDocument(CUPS_HTTP_DEFAULT, printName.toStdString().c_str(), jobId, "Imperion Document", CUPS_FORMAT_RAW, true);
